...
AstrBot 插件：每天 00:00 抓取 Pixiv 小说 R18 日榜，默认筛选中文作品，将每篇小说保存为独立 txt，并按每 10 篇一个合并转发推送到订阅群。也支持手动抓取 Pixiv R18 插画日榜前 50，并默认发送一个 PDF 和一个 zip 图片合集。
...
- `/pixiv_r18_illust_run`：立即抓取 Pixiv R18 插画日榜前 50，下载所有图片页，并发送 PDF 与 zip 合集到当前会话。
...
- `illust_send_as_zip`：插画以 PDF + zip 合集文件发送，默认开启。
